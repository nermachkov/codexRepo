...
1. Generate or create source PNG.
2. Quantize colors.
3. Extract connected regions.
4. Generate region map.
5. Generate black-and-white line art.
6. Generate metadata and labels.
7. Clean manually in an internal editor later if needed.

Recommended path:

- MVP uses raster artwork assets generated by our pipeline.
- Each region has an ID, color number, map color, area, bounds, and label position.
- A validation tool checks missing numbers, duplicate IDs, tiny regions, missing assets, and palette consistency.
- Later, build a content editor if cleanup and label placement become bottlenecks.
...
- Load `line_art.png`, `color_art.png`, `region_map.png`, and `metadata.json`.
- Use an offscreen color-coded hit map for region lookup.
- Reveal completed color pixels on Canvas.
...
- One gallery screen.
- One raster sample artwork.
- One Canvas-based coloring view.
- Palette selection.
- Tap-to-fill regions.
- Progress save.
